...
(function ($) {
	...
	var _initNode = function (setting, level, node, parentNode, isFirstNode, isLastNode, openFlag) {
		...
		if (typeof node[checkedKey] == "string") {
			node[checkedKey] = tools.eqs(node[checkedKey], "true");
		}

		node[checkedKey] = !!node[checkedKey];
		node.checkedOld = node[checkedKey];

		if (typeof node.nocheck == "string") {
			node.nocheck = tools.eqs(node.nocheck, "true");
		}

		node.nocheck = !!node.nocheck || (setting.check.nocheckInherit && parentNode && !!parentNode.nocheck);

		if (typeof node.chkDisabled == "string") {
			node.chkDisabled = tools.eqs(node.chkDisabled, "true");
		}

		node.chkDisabled = !!node.chkDisabled || (setting.check.chkDisabledInherit && parentNode && !!parentNode.chkDisabled);

		if (typeof node.halfCheck == "string") {
			node.halfCheck = tools.eqs(node.halfCheck, "true");
		}
		...
		node.getCheckStatus = function () {
			return data.getCheckStatus(setting, node);
		};
		...
		if (setting.check.chkStyle == consts.radio.STYLE
			...
			data.getRoot(setting).radioCheckedList.push(node);
		}
	};
	var _beforeA = function (setting, node, html) {
		if (setting.check.enable) {
	...
	var _view = {
		checkNodeRelation: function (setting, node) {
			...
			if (setting.check.chkStyle == consts.radio.STYLE) {
				...
				if (node[checkedKey]) {
					if (setting.check.radioType == consts.radio.TYPE_ALL) {
						...
						for (i = 0, len = parentNode[childKey].length; i < len; i++) {
				...
				} else if (setting.check.radioType == consts.radio.TYPE_ALL) {
		...
		makeChkClass: function (setting, node) {
			...
			if (node.chkDisabled === true) {
				fullStyle = consts.checkbox.DISABLED;
			} else if (node.halfCheck) {
				fullStyle = consts.checkbox.PART;
			} else if (setting.check.chkStyle == consts.radio.STYLE) {
				fullStyle = (node.check_Child_State < 1) ? consts.checkbox.FULL : consts.checkbox.PART;
			} else {
				fullStyle = node[checkedKey] ?
					((node.check_Child_State === 2 || node.check_Child_State === -1) ? consts.checkbox.FULL : consts.checkbox.PART) :
					((node.check_Child_State < 1) ? consts.checkbox.FULL : consts.checkbox.PART);
			}

			var chkName = setting.check.chkStyle + "_" + (node[checkedKey] ? consts.checkbox.TRUE : consts.checkbox.FALSE) + "_" + fullStyle;

			chkName = (node.check_Focus && node.chkDisabled !== true) ? chkName + "_" + consts.checkbox.FOCUS : chkName;

			return consts.className.BUTTON + " " + consts.checkbox.DEFAULT + " " + chkName;
		},
		...
		setSonNodeCheckBox: function (setting, node, value, srcNode) {
			...
			if (node != data.getRoot(setting) && node.chkDisabled !== true) {
				...
				if (setting.check.autoCheckTrigger
					&& node != srcNode
					&& node.nocheck !== true
